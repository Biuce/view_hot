    ...
    <div class="panel">
        <div class="panel-header">
            <h4 class="panel-title">
                @if(isset($id))
                    {{trans('equipment.editEquipment')}}
                @else
                    {{trans('equipment.newEquipment')}}
                @endif
            </h4>
        </div>

        <div class="panel-body">
            <form action="@if(isset($id)){{ route('admin::equipment.update', ['id' => $id]) }}@else{{ route('admin::equipment.save')}}@endif"
                  method="post" id="form">
                @if(isset($id)) {{ method_field('PUT') }} @endif
                {{ csrf_field() }}
                <div class="row form-group">
                    <div class="col-sm-1 text-sm-right">
                        <label class="col-form-label" for="standardInput">{{trans('equipment.levels')}}</label>
                    </div>
                    <div class="col-sm-11">
                ...
                <div class="row form-group">
                    <div class="col-sm-1 text-sm-right">
                        <label class="col-form-label" for="standardInput">{{trans('equipment.assorts')}}</label>
                    </div>
                    <div class="col-sm-11">
                        <select class="form-control" id="standardSelect" name="assort_id">
                            @foreach($assort ?? null as $v)
                                <option value="{{ $v['id'] }}"
                                        @isset($info) @if($v['id'] == $info->assort_id) selected @endif @endisset
                                >{{ $v['assort_name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row form-group">
                    ...
                    <div class="col-sm-11">
                        <div class="input-group form-group">
                            <input class="form-control" type="text" name="money" aria-describedby="icon-addon1"
                                   value="{{ $info->money ?? ''  }}">
                        </div>
                    </div>
                </div>
                    ...
                    @if(isset($id))
                        <div class="col-sm-12 text-sm-center">
                            <button class="btn btn-primary" type="submit"
                                    id="submitBtn">{{trans('equipment.editEquipment')}}</button>
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    layer.msg(result.msg, {shift: 1}, function () {
                        ...
                        if (result.redirect) {
                            location.href = '{{ route('admin::equipment.index') }}';
                        }
                    });
                },
