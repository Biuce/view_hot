        ...
        <div class="panel-header">
            <div class="d-flex flex-wrap align-items-center justify-content-between group-5">
                <h4 class="panel-title">
                    ...
                    <span>{{trans('assort.managers')}}</span>
                </h4>
                <div class="form-group">
                    <a href="{{ route('admin::assort.create') }}">
                        <button class="btn btn-primary" type="button"
                                id="submitBtn">{{trans('assort.newAssort')}}</button>
                    </a>
                </div>
            </div>
        </div>
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover" style="padding-bottom: 20px;">
                    ...
                    <tr class="long-tr">
                        <th>{{trans('assort.id')}}</th>
                        <th>{{trans('assort.assort_name')}}</th>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            {{--<a class="dropdown-item"--}}
                                               {{--href="{{ route('admin::assort.edit', ['id' => $v['id']]) }}">{{trans('general.edit')}}</a>--}}
                                            <a class="dropdown-item"
                                               href="{{ route('admin::assort.info', ['id' => $v['id']]) }}">{{trans('assort.info')}}</a>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button class="btn btn-primary"
                                                    onclick="deleteUser('{{ route('admin::assort.delete', ['id' => $v['id']]) }}')">{{trans('general.confirm')}}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
