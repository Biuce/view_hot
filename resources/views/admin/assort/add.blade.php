    ...
    <div class="panel">
        <div class="panel-header">
            <h4 class="panel-title">
                @if(isset($id))
                    {{trans('assort.editAssort')}}
                @else
                    {{trans('assort.newAssort')}}
                @endif
            </h4>
        </div>

        <div class="panel-body">
            <form action="@if(isset($id)){{ route('admin::assort.update', ['id' => $id]) }}@else{{ route('admin::assort.save')}}@endif"
                  method="post" id="form">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-sm-11">
                        <div class="input-group form-group">
                            <input class="form-control" type="text" name="assort_name" aria-describedby="icon-addon1"
                                   value="{{ $info->assort_name ?? ''  }}">
                        </div>
                    </div>
                </div>
                <div class="row row-10 align-items-center">
                    ...
                    @else
                        <div class="col-sm-12 text-sm-center">
                            <button class="btn btn-primary" type="submit" id="submitBtn">{{trans('assort.newAssort')}}</button>
                            <div style="display:inline;float:right;">
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    layer.msg(result.msg, {shift: 1}, function () {
                        ...
                        if (result.redirect) {
                            location.href = '{{ route('admin::assort.index') }}';
                        }
                    });
                },
